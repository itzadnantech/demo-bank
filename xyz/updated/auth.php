...
if ($_GET['action'] == "user_pass_reset") {
    ...
    $pasy = decryptData($r['password'], $r['salt']);
    ...
    if ($errorMsg == 0) {
        ...
        $res = encryptData($npassword);  
		$newpass = $res['enc_text'];
		$salt= $res['salt'];
        $query4 = $conn->query("UPDATE users SET password = '$newpass', salt = '$salt' WHERE id = '$userid'");
        echo "<script>
        Swal.fire({
        icon: 'success',
...
if ($_GET['action'] == "userPassResetConfirm") {
    if (isset($_POST)) {
        ...
        if ($errorMsg == 0) {
            ...
            $passy = $res['enc_text'];
            $salt= $res['salt'];
            $query = $conn->query("UPDATE users SET password = '$passy', salt = '$salt' WHERE id = '$user'");
            ...
            if (is_array($_FILES)) {
                if (is_uploaded_file($_FILES['passport']['tmp_name'])) {
                    ...
                    if (move_uploaded_file($sourcePath, $targetPath)) {
                        ...
                        $query = $conn->query("INSERT INTO users (firstname, lastname, middlename, state, country, city, zipcode, dob, address, phone, email, accountnumber, accounttype, usercurrency, accountbalance, imf, cot, secretCode, password, salt, approve, status, datecreated, income, occupation, passport) VALUES ('$firstname', '$middlename', '$lastname', '$state', '$country', '$city', '$zipcode', '$dob', '$address', '$phone', '$email', '$accountnumber', '$accounttype', '$usercurrency', '$accountbalance', '$imf', '$cot', '$secretCode', '$pass', '$salt', '0', 'active', '$dated', '$income', '$occupation', '$pic')");
...
if ($_GET['action'] == "loanRequest") {
    ...
    if ($_GET['action'] == "p2pStart") {
        ...
        if (empty($amount)) {
            ...
            borderError('green', 'amount');
        }

        if (empty($account)) {
            borderError('red', 'account');
        } else {
            borderError('green', 'account');
        }

        if ($daily_limit < checkLimit($userid) + $amount) {
            echo "<script>
    Swal.fire('Error Occured', 'Daily Transaction Limit Exceeded! Kindly contact our customer service at $siteemail For information on how to upgrade your', 'error');
    </script>";
            die();
        }
                                    ...
                                    <div class="buysell-overview">
                                        <ul class="buysell-overview-list">
                                            ...
                                            <li class="buysell-overview-item">
                                                ...
                                                <span class="pm-currency"><?php echo  number_format((5.8 + $amount), 2);
                                                                            echo " $money"; ?> </span>
                                            </li>
                                        </ul>
                                        <div class="sub-text-sm">*Our transaction fee are included.</div>
    ...
    if ($_GET['action'] == "ContinuePendingTransfer") {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            $_SESSION['bankname'] = $infos->bankname;
            $_SESSION['accountholder'] = $infos->accountholder;
            $_SESSION['routineNumber'] = $infos->routineNumber;
            $_SESSION['accountnumberB'] = $infos->accountnumberB;
            $_SESSION['description'] = $infos->description;
            $_SESSION['amount'] = $infos->amount;
